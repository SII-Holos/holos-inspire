...
export const inspireInference = tool({
...
async function handleCreate(params: any) {
  ...
  return {
    title: params.name,
    output: lines.join("\n"),
    metadata: { serving_id: servingId, workspace_id: ws.id, project_id: proj.id },
  }
}
...
async function handleDetail(params: any) {
  ...
  try {
    ...
    return {
      ...
      metadata: { error: "detail_failed", serving_id: params.serving_id },
    }
  }
...
async function handleStop(params: any) {
  if (!params.serving_id) {
    return {
      title: "缺少服务 ID",
  ...
  try {
    ...
    return {
      ...
      metadata: { serving_id: params.serving_id, action: "stopped" },
    }
  } catch (err: any) {
    return {
      title: "停止失败",
      output: `无法停止推理服务 ${params.serving_id}: ${err.message ?? err}`,
      metadata: { error: "stop_failed", serving_id: params.serving_id },
    }
  }
}
