...
function handleSummary(job: any, params: any, groups: MetricTimeSeries[], intervalSec: number) {
  const statusInfo = InspireNormalize.status(job.status ?? "")
  ...
  const lines = [
    ...
    `状态: ${statusInfo.family} | 时间窗口: ${TIME_RANGE_LABELS[params.time_range as TimeRange]} | 采样间隔: ${intervalSec}s`,
    "",
    assessment.status,
...
function handleRaw(job: any, params: any, groups: MetricTimeSeries[], intervalSec: number) {
  const statusInfo = InspireNormalize.status(job.status ?? "")
  const lines = [
...
export const inspireMetrics = tool({
  ...
  async execute(params, ctx) {
    ...
    if (statusInfo.family !== "running" && statusInfo.family !== "succeeded" && statusInfo.family !== "failed") {
      return {
        ...
        metadata: { job_id: params.job_id, status: statusInfo.family },
      }
    }
    ...
    if (!fetchResult) {
      return {
        ...
        metadata: { error: "missing_compute_group" },
      }
    }
    ...
    if (groups.length === 0) {
      return {
        ...
        metadata: { job_id: params.job_id, status: statusInfo.family, time_range: params.time_range },
      }
    }
